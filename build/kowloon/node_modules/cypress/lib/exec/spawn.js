...
var isDbusWarning = /Failed to connect to the bus:/;
...
var isCertVerifyProcBuiltin = /(^\[.*ERROR:cert_verify_proc_builtin\.cc|^----- Certificate i=0 \(OU=Cypress Proxy|^ERROR: No matching issuer found$)/;
...
var isMacOSElectronWebSwapCGLLayerWarning = /^objc\[\d+\]: Class WebSwapCGLLayer is implemented in both.*Which one is undefined\./;
var GARBAGE_WARNINGS = [isXlibOrLibudevRe, isHighSierraWarningRe, isRenderWorkerRe, isDbusWarning, isCertVerifyProcBuiltin, isMacOSElectronWebSwapCGLLayerWarning];
...
module.exports = {
  ...
  start: function start(args) {
    ...
    args = [].concat(_toConsumableArray(args), ['--cwd', process.cwd(), '--userNodePath', process.execPath, '--userNodeVersion', process.versions.node]);
    _.defaults(options, {
      dev: false,
      env: process.env,
      detached: false,
    ...
    var spawn = function spawn() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (stdioOptions.env.ELECTRON_RUN_AS_NODE) {
          ...
          args = [].concat(electronArgs, ['--'], _toConsumableArray(args));
        }
        if (startScriptPath) {
        ...
        var child = cp.spawn(executable, args, stdioOptions);
