...
function Xvfb(options) {
  options = options || {};
  this._display = options.displayNum ? ":".concat(options.displayNum) : null;
  this._reuse = options.reuse;
  this._timeout = options.timeout || options.timeOut || 2000;
  this._silent = options.silent;
  this._onStderrData = options.onStderrData || function () {};
...
Xvfb.prototype = {
          ...
          fs.exists(lockFile, function (exists) {
            ...
            if (exists) {
              ...
              if (totalTime > self._timeout) {
                ...
                var err = new Error('Could not start Xvfb.');
                err.timedOut = true;
                return cb && cb(err);
              } else {
        ...
        fs.exists(lockFile, function (exists) {
          if (!exists) {
            ...
            if (totalTime > self._timeout) {
              ...
              var err = new Error('Could not stop Xvfb.');
              err.timedOut = true;
              return cb && cb(err);
            } else {
  ...
  display: function display() {
    if (!this._display) {
      ...
      this._display = ":".concat(displayNum);
    }
    return this._display;
  },
  _setDisplayEnvVariable: function _setDisplayEnvVariable() {
    this._oldDisplay = process.env.DISPLAY;
    process.env.DISPLAY = this.display();
    debug('setting DISPLAY %s', process.env.DISPLAY);
  },
  _restoreDisplayEnvVariable: function _restoreDisplayEnvVariable() {
  ...
  _spawnProcess: function _spawnProcess(lockFileExists, onAsyncSpawnError) {
    var self = this;
    var onError = once(onAsyncSpawnError);
    var display = self.display();
    if (lockFileExists) {
      if (!self._reuse) {
        throw new Error("Display ".concat(display, " is already in use and the \"reuse\" option is false."));
      }
    } else {
      var stderr = [];
      var allArguments = [display].concat(self._xvfb_args);
  ...
  _killProcess: function _killProcess() {
    this._process.kill();
    this._process = null;
  },
  _lockFile: function _lockFile(displayNum) {
    displayNum = displayNum || this.display().toString().replace(/^:/, '');
    var filename = "/tmp/.X".concat(displayNum, "-lock");
