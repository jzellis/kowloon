...
app.post("/setup", async (req, res) => {
  ...
  const withFrontend = includeFrontend !== "false" && includeFrontend !== false;
...
`;
  ...
  // ── Write Caddyfile ─────────────────────────────────────────────────────────
  // Pure API paths always go to Express regardless of Accept header.
  // For everything else, browser requests (Accept: text/html) go to the
  // frontend container when it's included; API clients go to Express.
  const apiPaths = "/health /__health /config.json /auth/* /register /outbox /outbox/* /inbox /inbox/* /admin/* /files/* /.well-known/* /nodeinfo /invites/* /users/* /posts/* /circles/* /groups/* /bookmarks/* /notifications/* /pages/* /search /themes/* /preview /resolve";

  const caddyContent = withFrontend
    ? `# Managed by Kowloon — do not edit directly.
...
`;
  ...
  const frontendService = withFrontend
    ? `
  # ── Web interface ────────────────────────────────────────────────────────────
  frontend:
    image: ghcr.io/jzellis/kowloon-frontend:latest
