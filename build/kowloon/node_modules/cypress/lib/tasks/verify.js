...
var VERIFY_TEST_RUNNER_TIMEOUT_MS = +util.getEnv('CYPRESS_VERIFY_TIMEOUT') || 30000;
var checkExecutable = function checkExecutable(binaryDir) {
  ...
  return util.isExecutableAsync(executable).then(function (isExecutable) {
    ...
    return throwFormErrorText(errors.missingApp(binaryDir))(stripIndent(_templateObject || (_templateObject = _taggedTemplateLiteral(["\n      Cypress executable not found at: ", "\n    "])), chalk.cyan(executable)));
  });
};
...
function testBinary(version, binaryDir, options) {
  ...
  if (!options.force) {
    logger.log(stripIndent(_templateObject2 || (_templateObject2 = _taggedTemplateLiteral(["\n    It looks like this is your first time using Cypress: ", "\n    "])), chalk.cyan(version)));
  }
  logger.log();
...
var start = function start() {
  ...
  var parseBinaryEnvVar = function parseBinaryEnvVar() {
    ...
    logger.log(stripIndent(_templateObject3 || (_templateObject3 = _taggedTemplateLiteral(["\n      ", " You have set the environment variable:\n\n      ", "", "\n\n      This overrides the default Cypress binary path used.\n    "])), chalk.yellow('Note:'), chalk.white('CYPRESS_RUN_BINARY='), chalk.cyan(envBinaryPath)));
    logger.log();
    return util.isExecutableAsync(envBinaryPath).then(function (isExecutable) {
      ...
      if (!isExecutable) {
        return throwFormErrorText(errors.CYPRESS_RUN_BINARY.notValid(envBinaryPath))(stripIndent(_templateObject4 || (_templateObject4 = _taggedTemplateLiteral(["\n          The supplied binary path is not executable\n          "]))));
      }
    }).then(function () {
  ...
  return Promise["try"](function () {
    ...
    if (binaryVersion !== packageVersion) {
      ...
      logger.log();
      logger.warn(stripIndent(_templateObject5 || (_templateObject5 = _taggedTemplateLiteral(["\n\n\n      ", " Warning: Binary version ", " does not match the expected package version ", "\n\n        These versions may not work properly together.\n      "])), logSymbols.warning, chalk.green(binaryVersion), chalk.green(packageVersion)));
      logger.log();
    }
