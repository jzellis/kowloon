...
```jsonc
{
  ...
  "object": { … } | "id-string",                 // (required) NEW object payload (must include object.type) or an ID string for EXISTING objects
...
- **`object`**:
  - For **new objects** (e.g., `Create` a Post), submit fields **without** an `id`; the server assigns IDs. Include **`type`** with a valid subtype (see table below).
  - For **existing objects**, submit an **ID string** (local/remote) or, for `Follow`, `{ actorId: "@user@domain" }`.
  - **Reply**: when replying, use `Activity.type: "Reply"`, `objectType: "Post"`, set `object.type: "Reply"`, and include `object.inReplyTo` (ID string).
...
- `object.type` is **required** for all new objects; for auto-set cases (e.g., `Circle`, reaction objects), the server supplies the correct subtype.
...
## Changelog
- **2025‑10‑22**: Documented required `object.type` subtypes per `objectType`; added subtype table and examples; clarified `Reply` (`inReplyTo`).
